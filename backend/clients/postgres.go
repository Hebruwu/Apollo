...
type PostgresWriter interface {
	AddUser(ctx context.Context, user shared.User) error
}
...
func NewPostgresClient(dsn string) (PostgresClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to connect to database: %w due to %v", ErrFailedToConnect, err)
	}
...
func (p postgresClient) AddUser(ctx context.Context, user shared.User) error {
	...
	if err != nil {
		...
		if errors.As(err, &pgErr) && pgErr.Code == "23505" && pgErr.ConstraintName == "users_pkey" {
			return shared.ErrUsernameAlreadyExists
		}
